...
If a local repository exists and a GitHub repository needs to be created for that repository, run this code using the Terminal while *inside* the local Git repository folder (the root folder of the Git repository), that does the following actions:

- Creates a repo in the `seedcase-project` organization called `REPO` (rename to the folder name of the local repo).
- Sets the repo to public (not private) via the `--public`.
- Use the root folder as the local Git repository with `--source=.`.
- Include a description of the newly created repository with `--description=""`.
- Push the local repo to the new repo with `--push`.
- Disable the wiki with `--disable-wiki`.
- Set the URL for the homepage with `--homepage`, changing the `REPO` with the name of the root folder of the Git repo.
