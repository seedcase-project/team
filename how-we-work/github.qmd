...
- People are only added to a team when there is work to be done on a repository
  ...
  that everyone in Seedcase will get, so that only those relevant people are
  notified.
...
- A member is on a team if they are assigned work on the repository.
