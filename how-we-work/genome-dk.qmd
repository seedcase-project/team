...
users. To avoid us working in the same local repository on GenomeDK, we create
one version of the repository on GenomeDK per person in addition to having one
`main` version. The `main/` version of the repository will be the only one with
the smudged raw data.

For example, for a shared folder called `onlimit-study` and a repository called
`feasibility-data` and two users, `signekb` and `martonvago`, the folder
...
In the structure above, we have used the GitHub user names as the folder names.
Both repositories, `signekb` and `martonvago`, push and pull from GitHub.
...
4. Clone your repository using the VS Code terminal, if you haven't already:
   ...
   ```bash {filename="Terminal"}
      cd <.../my/projects>
      git clone <repo-url> <username>
      cd <username>
   ```

   The <username> (your GitHub user name) above will name the local repository
   with your username. E.g., cloning the repository `feasibility-data` with the
   username `signekb` will name the local repository folder `signekb` instead of
   its remote name `feasibility-data`. This way, we avoid having to work
   multiple people in the same local version of the repository on GenomeDK. See
   the [Collaborating and working in a shared folder](#collaborating) section
