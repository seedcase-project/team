...
working with, [DP-Next](https://dp-next.github.io) and [ON-LiMiT](https://on-limit.github.io).

## Access

Employee's of Aarhus University or Aarhus University Hospital can ask for an account on GenomeDK,
which is done via the [request account](https://console.genome.au.dk/user-requests/create/) portal. 
For now, we will need to select the "Open" zone. You need
to state your preferred username and password even though the
form seems to say to only do it if you want to re-activate an old account.

## Setting up

Once you get approved for access you can run through the
[documentation](https://genome.au.dk/docs/), which is extensive and
quite detailed. Because GenomeDK is a Linux-based server,
you setup, access, and do many actions through a shell terminal. 
...
We will primarily be working with an SSH
connection, with the guidance on how to
set up a public key to connect to the server is fairly easy to
follow.

You can then also access the files [local](https://genome.au.dk/docs/working-with-data/#mounting) 
to be able to use text editors as normal.
...
[documentation](https://genome.au.dk/docs/getting-started/). Once done, you can
