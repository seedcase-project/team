...
repository alongside the Git LFS store and a `env` folder containing the `.env`
file(s). For example, for a repository named `feasibility-data`, the folder
...
the same local repository on GenomeDK, each of us create a clone in our our own
user space. To set up your own repository follow these steps:

1. Go to GenomeDK, either via an [SSH connection](#ssh), e.g., via [VS
   Code](genome-dk.qmd#developing-with-vs-code) or through the [GenomeDK
   desktop](https://desktop.genome.au.dk/).

2. Ensure you have `git lfs` installed, see [how to install Git
   LFS](#installing-git-lfs) above. If you've installed it with conda, make sure
   to activate your conda environment before continuing, so the Git LFS file
   pointers get resolved correctly.
...
having access to the Git LFS store and a shared `.env` file.

<!-- TODO: Since there's some functions needed in the repository to access the correct env file, should we move that code to a collected repo, e.g., `shears`, that we can load and use across data packages? -->
