...
## Collaborating and working in a shared folder {#collaborating}

When working in a shared folder on GenomeDK, we work in our own local repository
clones in our own user space. In the shared folder, we have a clone of the
repository alongside the Git LFS store and an `env/` folder containing the
[`.env` file](https://env.dev/guides/dotenv)(s). For example, for a repository
named `feasibility-data`, the folder structure in the shared folder looks like
this:
...
Any changes made (e.g., editing files, or checking out a different branch) in
the shared repository will be visible for all users. So, to avoid us working in
the same shared repository on GenomeDK, we each create a clone in our own user
space. To set up your own repository, follow these steps:

1. Go to GenomeDK, either via an [SSH connection](#ssh), e.g., with [VS
   Code](genome-dk.qmd#developing-with-vs-code), or via the [GenomeDK
   desktop](https://desktop.genome.au.dk/).
...
Now, you're set up to work in your individual clone of the repo while still
having access to the Git LFS store.

To be able to use the shared `.env` file in your clone, the repository needs to
be set up to load the correct file. E.g., see how we've set it up in the
ON-LiMiT [feasibility-data](https://github.com/onlimit-study/feasibility-data)
repository.

<!-- TODO: Since there's some functions needed in the repository to access the correct env file, should we move that code to a collected repo, e.g., `shears`, so we can load and use across data packages, or simply refer to it here?? -->
