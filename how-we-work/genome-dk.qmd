...
3.  Create a folder on GenomeDK that everyone working on the Data
    Package can access and edit: this will be the LFS store where all
...
5.  Create an `.lfsconfig` file in the Data Package root folder and add
    the absolute path to the LFS store:
...
6.  Define a rule for tracking all Parquet files and all data in the
    `raw/` folder by running the following commands from the root of the
    Data Package:
