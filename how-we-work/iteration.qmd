...
-   Will plan iterations so that they either end on a major
    holiday or are right in the middle of it. If possible, don't plan the start of
...
An iteration starts with a first planning meeting that is
...
meeting. Before the planning meeting, the team lead will move any existing tasks
...
team goes through the tasks on the board together. We'll discuss and decide or agree
...
constraints, including days/hours required for other projects and any upcoming days off.
...
(preferably on the same date, but after lunch to ensure a proper break before moving to a new iteration).
:::
...
-   Work on our assigned tasks (generally focus on higher priority tasks first) and coordinate with others if need be
...
in person. The time and place of this meeting will have been scheduled during the planning meeting.
...
-   Assign a timekeeper to keep us on track and so we avoid spending too long on
    any one topic.
-   Show off and (optionally) demo the output of the iteration all together as a team.
...
-   Do a "retrospective", where we individually write down how things
    ...
    system ([Figma](https://www.figma.com/) is really nice for this). These notes will be collected into topics and we will add action items to each of the topics. The team lead will add these action items to issues or posts on how we work.
